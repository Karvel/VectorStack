...
public:
	...
	// CONSTRUCTOR
	stack() { const size_type size() = 0; } //I specified size() as a constant to prevent a left operand must be an lvalue error.
	// MODIFICATION MEMBER FUNCTIONS
	void push(const Item& entry);
	void pop();
	// CONSTANT MEMBER FUNCTIONS
	bool empty() const { return data.empty; } //inline
	size_type size() const { return data.size(); } //inline
	Item top() const;
private:
	vector<Item> data;
};//end stack

//#include "mystack.cpp" - removed this due to linker issues
